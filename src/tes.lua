...
function ArtHub:MakeNotify(NotifyConfig)
	...
	NotifyConfig.Color = NotifyConfig.Color or Color3.fromRGB(0, 95, 40)
...
function ArtHub:Stat(config)
	...
	local CustomLogo = "rbxassetid://122485275798323"
	local AllPossibleStats = { "Ping", "CPU", "FPS", "Mem" }
	local currentDisplay = config.Display or { "Ping", "CPU" }

	local StatsGui = CoreGui:FindFirstChild("ArtHubStatsGui") or Instance.new("ScreenGui")
	StatsGui.Name = "ArtHubStatsGui"
	StatsGui.DisplayOrder = 999
	StatsGui.ResetOnSpawn = false
	if not StatsGui.Parent then pcall(function() StatsGui.Parent = CoreGui end) end

	local MainFrame = Instance.new("Frame")
	MainFrame.Name = "StatDisplay"
	MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
	MainFrame.Position = config.Position or UDim2.new(0, 80, 0, 80)
	...
	StatsContent.Name = "B_StatsList"
	StatsContent.LayoutOrder = 2
	StatsContent.BackgroundTransparency = 1
	StatsContent.AutomaticSize = Enum.AutomaticSize.XY
	StatsContent.Parent = MainFrame

	local ContentList = Instance.new("UIListLayout", StatsContent)
	ContentList.FillDirection = Enum.FillDirection.Vertical
	ContentList.HorizontalAlignment = Enum.HorizontalAlignment.Left
	ContentList.Padding = UDim.new(0, 2)

	local StatLabels = {}
	for _, name in ipairs(AllPossibleStats) do
		...
		for _, d in ipairs(currentDisplay) do if d == name then isDefault = true end end
		Label.Visible = isDefault
	...
	local fpsBuffer = {}
	local cpuBuffer = {}
	local maxBufferSize = 45
	local lastTextUpdateTime = 0
	local textUpdateDelay = 0.3

	local function GetStatColor(value, type)
		if type == "Ping" then
			if value <= 100 then
				return Color3.fromRGB(85, 255, 127)
			elseif value <= 300 then
				return Color3.fromRGB(255, 255, 127)
			else
	...
	local Connection = RunService.Heartbeat:Connect(function(dt)
		...
		table.insert(cpuBuffer, dt * 1000)

		if #fpsBuffer > maxBufferSize then table.remove(fpsBuffer, 1) end
		if #cpuBuffer > maxBufferSize then table.remove(cpuBuffer, 1) end

		if tick() - lastTextUpdateTime >= textUpdateDelay then
			if StatLabels["Ping"].Visible then
				local p = math.floor(StatsService.Network.ServerStatsItem["Data Ping"]:GetValue())
				StatLabels["Ping"].Text = "Ping: " .. p .. "ms"
				StatLabels["Ping"].TextColor3 = GetStatColor(p, "Ping")
			end

			if StatLabels["FPS"].Visible then
				local fpsSum = 0
				for i = 1, #fpsBuffer do fpsSum = fpsSum + fpsBuffer[i] end
				local avgFps = math.floor(fpsSum / #fpsBuffer)
				StatLabels["FPS"].Text = "FPS: " .. avgFps
				StatLabels["FPS"].TextColor3 = GetStatColor(avgFps, "FPS")
			end

			if StatLabels["Mem"].Visible then
				local m = math.floor(StatsService:GetTotalMemoryUsageMb())
				StatLabels["Mem"].Text = "Mem: " .. m .. "MB"
				StatLabels["Mem"].TextColor3 = GetStatColor(m, "Mem")
			end

			if StatLabels["CPU"].Visible then
				local cpuSum = 0
				for i = 1, #cpuBuffer do cpuSum = cpuSum + cpuBuffer[i] end
				local avgCpu = cpuSum / #cpuBuffer
				StatLabels["CPU"].Text = string.format("CPU: %.1fms", avgCpu)
				StatLabels["CPU"].TextColor3 = GetStatColor(avgCpu, "CPU")
			end

			lastTextUpdateTime = tick()
		end
	end)

	return {
		Instance = MainFrame,
		SetVisible = function(self, state) MainFrame.Visible = state end,
		UpdateDisplay = function(self, list)
			for name, label in pairs(StatLabels) do
				local show = false
				for _, v in ipairs(list) do if v == name then show = true end end
				label.Visible = show
			end
		end,
		Destroy = function(self)
			Connection:Disconnect(); MainFrame:Destroy()
		end
	}
end
...
function ArtHub:Window(GuiConfig)
	...
	Close.Activated:Connect(function()
		...
		Dialog.Image = "rbxassetid://94291094948706"
	...
	function GuiFunc:ToggleUI()
		...
		MainButton.Position = UDim2.new(0, 15, 0, 121)
